...
const textoDesencriptado=desencriptar(inputTexto.value)
mensaje.value=textoDesencriptado;
//inputTexto.value=textoDesencriptado     //y para que la imagen siempre se mantenga quitariamos o comentariamos el background image no aqui sino desde la otra función la de ecriptar para que desde origen no aparezca
//mensaje.style.backgroundImage="none"; //seria textoDesencriptado.style(APLICAMOS ESTILOS CSS AQU EN JS) para que todo se haga en la caja 1 y //cuando se presione el boton de encriptar el logo no se muestre 
inputTexto.value = "";   //y cuando se pase el texto limpie la caja de texto  Nota si comentamos este ambos textos se mantienen, pero asi no lo pide la práctica se tiene que limpiar a fuerzas la caja de texto al pasarle el texto(inputTexto) a la otra caja (mensaje)
}   
...
function encriptar(stringEncriptada) {     //declara la matriz
    let matrizCodigo = [["e", "enter"], ["i", "imes"], ["a", "ai"], ["o","ober"], ["u","ufat"]];
    stringEncriptada = stringEncriptada.toLowerCase();  //la convierte a minusculas

    for(let i=0; i < matrizCodigo.length; i++) {  //recorre el arreglo
        if(stringEncriptada.includes(matrizCodigo[i][0])) {  
            stringEncriptada = stringEncriptada.replaceAll(matrizCodigo[i][0], matrizCodigo[i][1])
        }                // busca el index o indice en la posicion 0 y la convierte en lo que tiene el arreglo pero ahora en la posicion 1
    }
    return stringEncriptada;
...
En el If, la parte que falta solo es matrizCodigo[i][0])
para que nuevamente haga el cambio al original
Ej de  de ailufatrai  a alura
